...
- (void)createDB {
    int x = sqlite3_open_v2([RSUtils getDBPath], &(self->_database), SQLITE_OPEN_CREATE | SQLITE_OPEN_READWRITE | SQLITE_OPEN_FULLMUTEX, nil);
    if (x == SQLITE_OK) {
        // opened correctly
    } else {
        [RSLogger logError: [[NSString alloc] initWithFormat:@"Failed to open db: %d", x]];
    }
}
